...
function buildPreviewCard(report, cfg = getConfig()) {
  ...
  if (report.agents.length) {
    const lines = report.agents.map(a =>
      `${a.tierEmoji} *${a.name}*${a.top ? ' :crown:' : ''} — *${a.count}* open >24h · oldest *${a.oldestDays}d*`)
    if (report.unassigned.count) {
      lines.push(`:grey_question: *Unassigned* — *${report.unassigned.count}* open >24h · oldest *${report.unassigned.oldestDays}d*`)
    }
    // Chunk into sections so each stays under Slack's 3000-char limit.
    let buf = []
    let len = 0
    let first = true
    const flush = () => {
      if (!buf.length) return
      const heading = first ? '*By agent* (most aging first)\n' : ''
      blocks.push({ type: 'section', text: { type: 'mrkdwn', text: heading + buf.join('\n') } })
      first = false; buf = []; len = 0
    }
    for (const line of lines) {
      if (len + line.length > 2600) flush()
